...
-- @pAgentCodeIn  = NULL， Insert OR Update 所有戶口到dbo.mVIPPerosn
-- @pAgentCodeIn <> NULL， Insert OR Update 指定戶口到dbo.mVIPPerosn
CREATE PROC [util].[RecalVIPPersonForAgent]
        ...
        BEGIN TRY
            ...
            BEGIN
                CREATE TABLE #vVIPPerson (
                    wRowNum                 BIGINT, -- 此處不能用處自增id，因為後面會根據當前跟進部門T掉不是VIP跟進的Record
                    RowID                   BIGINT,
                ...
                SELECT
                    ...
                    wPersonIdentity = 'VIP_AGENT',
                    wGender = ma.wSex, 
                    wAuthorizerAgentCodeIn = ma.wAgentCodeIn,  -- 此值需要填寫，需要用wAgentCodeIn + wAuthorizerAgentCodeIn，區別戶口與授權人
                    wAuthorizerIdentity    =  NULL, -- 暫時為空
                    wRelationship      = '00', 
                ...
                WHERE mp.RowID IS NULL
                    AND ma.wStatus = 'A'
                    AND ma.wType = 'AGENT'
                    AND (@pAgentCodeIn IS NULL OR @pAgentCodeIn = ma.wAgentCodeIn) -- 戶口
                OPTION(RECOMPILE);
                ...
                -- 刪除wStatus='T'的Record不要Insert到mVIPPerson
                DELETE FROM #vVIPPerson WHERE wStatus = 'T';

                -- Set MD跟進 VIP客戶身份
                UPDATE mp
                SET mp.wPersonIdentity = 'MD_AGENT' -- MD星級戶口
                FROM #vVIPPerson mp
            ...
            BEGIN
                -- 更新戶口基本資料(wStatus='A')
                UPDATE mp
                SET wAgentCodeIn = ma.wAgentCodeIn,
                    wPersonName = ma.wCName,
                    -- wPersonIdentity = 'AGENT',
                    wGender = ma.wSex, 
                    wAuthorizerAgentCodeIn = ma.wAgentCodeIn,
                    -- wAuthorizerIdentity = NULL, 
                    wBirthDate = ISNULL(ma.wBirthDate, '0001-01-01'), 
                ...
                INNER JOIN RollsMary.dbo.mAgent ma ON ma.wAgentCodeIn = mp.wAgentCodeIn AND ma.wAgentCodeIn = mp.wAuthorizerAgentCodeIn AND mp.wIsAuthorizer = 'Y' AND mp.wStatus = 'A'
