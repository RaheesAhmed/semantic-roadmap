CREATE PROCEDURE [spq].[GetSMSAirTicketBooking]
    @pBookingRid BIGINT ,
    @pGuid VARCHAR(50) ,
    @pLangCd VARCHAR(30)
AS
    BEGIN
        ...
        SET @sWaitTicketFeeRid = ( SELECT TOP(1) sub.RowID
                                   FROM dbo.mExpenseSubtype sub
                                   INNER JOIN dbo.mExpensetype t ON sub.wExpenseTypeId =t.RowID AND t.wCode ='003' AND t.wExpCat = 'TR'
                                   WHERE sub.wCode = '007'
        );
        ...
        INNER JOIN dbo.eBooking b ON bat.wBookingRid = b.RowID
