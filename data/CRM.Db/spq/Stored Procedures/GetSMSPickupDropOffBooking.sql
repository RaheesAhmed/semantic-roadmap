CREATE PROCEDURE [spq].[GetSMSPickupDropOffBooking]
    @pBookingRid BIGINT ,
    @pGuid VARCHAR(50) ,
    @pLangCd VARCHAR(30)
AS
    BEGIN
        ...
        SELECT  wIsMDAgent = @sIsMDAgent, -- MD轉介戶口
                ...
                dap.wCity AS wDepartureCity ,
                wDepartureAirportName = CASE WHEN @pLangCd = 'en-gb' THEN dap.wEName ELSE dap.wCName END ,
                bp.wDepartDt ,
                aap.wCity AS wArrivalCity ,
                wArrivalAirportName = CASE WHEN @pLangCd = 'en-gb' THEN aap.wEName ELSE aap.wCName END ,
                bp.wArrivalDt ,
                bp.wFlightNo ,
                bp.wApplyDt
        FROM dbo.eBookingPickUpService bp
        LEFT JOIN CRM.dbo.eBooking b ON bp.wBookingRid = b.RowID
        LEFT JOIN CRM.dbo.mTravelAgency ta ON ta.RowID = bp.wTravelAgencyRid
        ...
        LEFT JOIN CRM.dbo.mAirport dap ON dap.RowID = bp.wDepartAirport
        LEFT JOIN CRM.dbo.mAirport aap ON aap.RowID = bp.wDestination
        WHERE b.RowID = @pBookingRid;
    END;