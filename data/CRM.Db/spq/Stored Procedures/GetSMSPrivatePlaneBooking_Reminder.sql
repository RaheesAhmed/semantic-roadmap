...
CREATE PROCEDURE [spq].[GetSMSPrivatePlaneBooking_Reminder]
	/*
		EXEC spq.GetSMSPrivatePlaneBooking_Reminder '1000010180', '<DataSet><Record wBookingRid="10000000011554"/></DataSet>', '', 'zh-TW'		
	*/
        ...
        WITH    ctePassengerData
            ...
            SELECT  bpp.wBookingRid ,
                 ...
                 --   bpp.wHasWifi ,
                    ...
                    --bpp.wCancelDate ,
                    --bpp.wCancelReason ,
                    CAST(( CASE WHEN sc.wCode IN ( 'CR', 'FY-MFM' ) THEN 'SUNTRAVEL'
                                ELSE 'SUNGROUP'
                ...
                --    tcp.wName AS wTicketCollectionPointName ,
                    ...
                    CASE WHEN @pLangCd = 'en-gb' THEN u.wName
                         ELSE u.wCName
                    END AS wUpdByName ,
                    ISNULL(STUFF(( SELECT   N'、' + ctePD.wName
                                   FROM     ctePassengerData ctePD
                                 FOR
                                   XML PATH('')
                                 ), 1, 1, ''), '') AS wPassengerName ,
                    ISNULL(STUFF(( SELECT   ',' + ctePD.wClientTicketNo
                                   FROM     ctePassengerData ctePD
                                 FOR
                                   XML PATH('')
