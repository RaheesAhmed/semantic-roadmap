...
CREATE PROCEDURE [spq].[GetRptBookingCheckInService]    
(    
...
BEGIN    
	...
	SELECT    
		...
		ebcis.wOrderNo AS OrderNo,					--單號/確認號
		ebcis.wFlightNo AS FlightNo,				--航班編號
		DAP.wAirPortName AS DepartureCity,			--出發地點
		AAP.wAirPortName AS ArivalCity,				--目的地點
		ebcis.wDepartDt AS DepartureDateTime,		--航班時間
		ebcis.wArrivalDt AS ArrivalDateTime,
		ebcis.wQuantity AS PassengerQuantity,		--人数
		...
		ebcis.wReceiptNo AS ReceiptNo,				--現金單號
		ebcis.wRemark AS Remark,						--備註
		mtp.wRefNo AS TravePkgRefNo,				--相關套票編號
		ebcis.wBookingStatus AS BookingStatus		--訂單狀態
	FROM  dbo.eBookingCheckInService ebcis
	INNER JOIN dbo.eBooking AS eb ON eb.RowID = ebcis.wBookingRid
	...
	LEFT JOIN mAirport AS DAP ON ebcis.wDepartAirport = DAP.RowID
	LEFT JOIN mAirport AS AAP ON ebcis.wDestination = AAP.RowID
