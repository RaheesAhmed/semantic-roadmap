CREATE PROCEDURE [spq].[GetSMSVisaBooking]
    @pBookingRid BIGINT ,
    @pGuid VARCHAR(50) ,
    @pLangCd VARCHAR(30)
AS
    BEGIN
        ...
        -- 總值
        DECLARE @sTotalAmt NUMERIC(18, 4);
        -- 總成本
        DECLARE @sTotalCost NUMERIC(18, 4);
        ...
        SET @sPassengerName = STUFF(
            (SELECT CONCAT(N'、', IIF(@pLangCd = 'en-gb', p.wEName, p.wCName))
        ...
        -- 總值、總成本
        SELECT @sTotalAmt = SUM(wAmount),
               @sTotalCost = SUM(wCost)
        ...
        SELECT  wIsMDAgent = @sIsMDAgent, -- MD轉介戶口
                ...
                tcp.wAddress AS wTicketCollectionPointAddress ,
                b.wCrtDt ,
                b.wCrtBy ,
        ...
        LEFT JOIN CRM.dbo.eBooking b ON bv.wBookingRid = b.RowID
        LEFT JOIN CRM.dbo.eVisaCollection vc ON vc.wBookingRid = bv.wBookingRid
        LEFT JOIN CRM.dbo.mTicketCollectionPoint tcp ON tcp.wCode = vc.wTicCollPoint AND tcp.wStatus = 'A'
