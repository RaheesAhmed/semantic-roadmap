CREATE PROCEDURE [spq].[GetSMSShowBooking]
    @pBookingRid BIGINT ,
    @pGuid VARCHAR(50) ,
    @pLangCd VARCHAR(30)
AS
    BEGIN
        ...
        SELECT  wIsMDAgent = @sIsMDAgent, -- MD轉介戶口
                ...
                tc.wTicCollPoint ,
                mtc.wName AS wTicCollPointName ,
                mtc.wAddress AS wTicCollPointAddress ,
                tc.wIsCollected ,
                tc.wCollDate ,
                tc.wCollStaff ,
                tc.wCollRemark ,
                tc.wSeqNo ,
                stp.wTicketType AS wTicketPriceTypeName ,
                bst.wQuantity AS wShowTicketQuantity ,
                stp.wTicketType ,
                tp.wClassCd ,
        ...
        INNER JOIN CRM.dbo.eBooking b ON bs.wBookingRid = b.RowID
        INNER JOIN dbo.mShow s ON s.RowID = bs.wShowRid
        LEFT JOIN CRM.dbo.eTicketCollection tc ON tc.wBookingRid = bs.wBookingRid
        LEFT JOIN CRM.dbo.eBookingShowTicket bst ON bst.wBookingShowRid = bs.RowID
        LEFT JOIN CRM.dbo.mShowTicketPrice stp ON stp.RowID = bst.wShowTicketPriceRid
        LEFT JOIN CRM.dbo.eTicketPricing tp ON tp.RowID = bst.wShowTicketPriceRid
        ...
        LEFT JOIN CRM.dbo.mTicketCollectionPoint mtc ON mtc.wCode = tc.wTicCollPoint
        WHERE   b.RowID = @pBookingRid;
    END;