...
CREATE PROCEDURE [spa].[SetPassengerTravelDoc_ForAir] 
(  
		@pXML XML ,  
...
AS  
    BEGIN  
	 ...
	 ----SELECT 0 AS wSeqNo,* FROM dbo.ePassengerTravelDocDetails   
     ...
     --    
       ...
       BEGIN TRY
		...
		IF @pBookingType='PASSENGER'
		BEGIN
			DELETE FROM [ePassengerTravelDocDetail] WHERE wPassengerDetailsRid IN (SELECT wPassengerDetailsRid FROM #sDataSet_SetPassengerTravelDocDetail);
		END
