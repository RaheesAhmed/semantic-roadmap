...
CREATE PROCEDURE [spa].[SetItemSerial]
    (
      @pXML XML ,
      @pMainCompNo INT ,
...
AS
    BEGIN
        ...
        DECLARE @sThisTableName VARCHAR(50) = 'mItemSerial' ,
        ...
        SELECT  wRowNum = ROW_NUMBER() OVER ( ORDER BY wUpdDt ) ,
                *
        INTO    #sDataSet_SetItemSerial
        FROM    OPENXML (@sDocHandle, 'DataSet/Record', 1)
	WITH (
         RowID BIGINT, wItemRid BIGINT, wPurchaseRid BIGINT,wSerialNo VARCHAR(50), wStatus CHAR(1), wCrtDt DATETIME2, wCrtBy BIGINT, wUpdDt DATETIME2, wUpdBy BIGINT, RecordState VARCHAR(1));        
        BEGIN TRY	                
            ...
            IF EXISTS ( SELECT  1
                        FROM    #sDataSet_SetItemSerial
                        WHERE   RecordState = 'I' )
                BEGIN
					-- Set RowID by Sequence
                    UPDATE  #sDataSet_SetItemSerial
                    SET     RowID = 0
                    WHERE   RecordState = 'I';
            ...
            IF EXISTS ( SELECT  1
                ...
                BEGIN
                    UPDATE  is_t
                    SET     wItemRid = tmp.wItemRid ,
                            wPurchaseRid = tmp.wPurchaseRid ,
                            wSerialNo = tmp.wSerialNo ,
                            wStatus = tmp.wStatus ,
                            wUpdDt = @sNow ,
                            wUpdBy = tmp.wUpdBy
                    FROM    [dbo].[mItemSerial] is_t
                            INNER JOIN #sDataSet_SetItemSerial tmp ON is_t.RowID = tmp.RowID
        ...
        IF OBJECT_ID('tempdb..#sDataSet_SetItemSerial') IS NOT NULL
            DROP TABLE #sDataSet_SetItemSerial;
    END;