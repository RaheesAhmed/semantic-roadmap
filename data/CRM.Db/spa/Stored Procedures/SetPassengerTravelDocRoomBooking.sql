...
CREATE PROCEDURE [spa].[SetPassengerTravelDocRoomBooking] 
(  
		@pXML XML ,  
     ...
     --    
       ...
       BEGIN TRY
        ...
        IF @pActionType = 'I'  
			...
			WHERE wPassengerDetailsRid IN (
			SELECT pd.RowID FROM ePassengerDetails pd 
			INNER JOIN #sDataSet_SetPassengerTravelDocDetail tmp ON tmp.wSeqNo =pd.wSeqNo AND wRoomBookingRid =@pRommBookingRid)
		                
		UPDATE tmp
		SET tmp.wPassengerDetailsRid=pass.RowID
		FROM #sDataSet_SetPassengerTravelDocDetail tmp
		INNER JOIN (SELECT RowID,wSeqNo FROM dbo.ePassengerDetails WHERE wRoomBookingRid=@pRommBookingRid AND wStatus='A') pass ON pass.wSeqNo =tmp.wSeqNo;
