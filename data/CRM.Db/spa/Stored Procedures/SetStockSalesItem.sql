...
CREATE PROCEDURE [spa].[SetStockSalesItem]
    (
      @pXML XML ,
      @pMainCompNo INT ,
...
AS
    BEGIN
        ...
        DECLARE @sThisTableName VARCHAR(50) = 'eStockSalesItem' ,
        ...
        SELECT  wRowNum = ROW_NUMBER() OVER ( ORDER BY wUpdDt ) ,
                *
        INTO    #sDataSet_SetStockSalesItem
        FROM    OPENXML (@sDocHandle, 'DataSet/Record', 1)
	WITH (
         RowID BIGINT, wStockSalesDtlRid BIGINT, wPurchaseRid BIGINT, wQty INT, wUnitCostHKD NUMERIC(18,4), wStatus CHAR(1), wCrtDt DATETIME2, wCrtBy BIGINT, wUpdDt DATETIME2, wUpdBy BIGINT, RecordState VARCHAR(1));        
        BEGIN TRY	                
            ...
            IF EXISTS ( SELECT  1
                        FROM    #sDataSet_SetStockSalesItem
                        WHERE   RecordState = 'I' )
                BEGIN
					-- Set RowID by Sequence
                    UPDATE  #sDataSet_SetStockSalesItem
                    SET     RowID = 0
                    WHERE   RecordState = 'I';
                    ...
                    INSERT  INTO [dbo].[eStockSalesItem]
                            ( RowID ,
                              wStockSalesDtlRid ,
                              wPurchaseRid ,
                              wQty ,
            ...
            IF EXISTS ( SELECT  1
                ...
                BEGIN
                    UPDATE  ssi_t
                    SET     wStockSalesDtlRid = tmp.wStockSalesDtlRid ,
                    ...
                    FROM    [dbo].[eStockSalesItem] ssi_t
                            INNER JOIN #sDataSet_SetStockSalesItem tmp ON ssi_t.RowID = tmp.RowID
        ...
        IF OBJECT_ID('tempdb..#sDataSet_SetStockSalesItem') IS NOT NULL
            DROP TABLE #sDataSet_SetStockSalesItem;
    END;