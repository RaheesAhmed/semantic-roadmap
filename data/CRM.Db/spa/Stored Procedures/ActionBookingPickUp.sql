CREATE PROCEDURE [spa].[ActionBookingPickUp]
    @pXMLBooking XML = NULL ,
    @pXMLBookingPickUp XML = NULL ,
...
AS
    BEGIN  
	 ...
	 --Select CAST(Count(*) as BIGINT) as wBookingRid, CAST(Count(*) as BIGINT) as wBookingTourRid  from eBookingAirTicket  
      ...
      -- Try to make the transaction scope as small as possible to reduce locking  
            ...
            IF @pXMLBooking IS NOT NULL
                BEGIN
				 -- Validate Status Management Update Booking field Start		
                    EXEC @errorMsg= [dbo].[fnValidateBookingPickUpServiceByStatus] @pXml = @pXMLBooking, @pActionType = @pActionType;
                    IF @errorMsg <> ''
            ...
            IF @pActionType = 'I'
                BEGIN
                    IF @pXMLPassengerDetail IS NOT NULL
                        SET @pAddRecord = 1;
            ...
            IF @pAddRecord = 1
                THROW 50001, 'Can Not Insert Passenger, Passenger Travel Doc or Voucher before save PickUp booking', 1;	

            IF @pActionType = 'I'
                AND @pXMLPassengerDetail IS NOT NULL
                AND @pXMLPassengerTravelDoc IS NULL
                THROW 50001, 'Travel Doc is Missing', 1;


            IF @pXMLPassengerDetail IS NOT NULL
                EXEC [spa].[SetPassengerDetailsCheckInServices] @pXMLPassengerDetail, @pActionType, @pMainCompNo, @pNonceToken, 'N', @pBookingRid, 0, -1, @pErrCode OUTPUT, @pErrMsg OUTPUT;

			IF @pXMLBookingPickUp IS NOT NULL
                EXEC [spa].[SetBookingPickUp] @pXMLBookingPickUp, @pActionType, @pMainCompNo, @pNonceToken, 'N', @pBookingRid, @pBookingTourRid OUTPUT, @pErrCode OUTPUT, @pErrMsg OUTPUT;
            ...
            IF @pBookingRid IS NOT NULL AND @pBookingRid > 0 BEGIN
                EXEC util.WriteMaryAgentActivitiesApiLog @pBookingRid = @pBookingRid, @pBookingType = 'PICKUP_SERVICE';
            END;
