...
CREATE PROCEDURE [spa].[SetStockMovementItem]
    (
      @pXML XML ,
      @pMainCompNo INT ,
...
AS
    BEGIN
        ...
        DECLARE @sThisTableName VARCHAR(50) = 'eStockMovementItem' ,
        ...
        SELECT  wRowNum = ROW_NUMBER() OVER ( ORDER BY wUpdDt ) ,
                *
        INTO    #sDataSet_SetStockMovementItem
        FROM    OPENXML (@sDocHandle, 'DataSet/Record', 1)
    WITH (
         RowID BIGINT, wStockMovementDtlRid BIGINT, wPurchaseRid BIGINT, wQty INT, wUnitCostHKD NUMERIC(18,4), wStatus CHAR(1), wCrtDt DATETIME2, wCrtBy BIGINT, wUpdDt DATETIME2, wUpdBy BIGINT, RecordState VARCHAR(1));        
        BEGIN TRY	                
            ...
            IF EXISTS ( SELECT  1
                        FROM    #sDataSet_SetStockMovementItem
                        WHERE   RecordState = 'I' )
                BEGIN
                    -- Set RowID by Sequence
                    UPDATE  #sDataSet_SetStockMovementItem
                    SET     RowID = 0
                    WHERE   RecordState = 'I';
                    ...
                    INSERT  INTO [dbo].[eStockMovementItem]
                            ( RowID ,
                              wStockMovementDtlRid ,
                              wPurchaseRid ,
                              wQty ,
                              wUnitCostHKD ,
                              wStatus ,
            ...
            IF EXISTS ( SELECT  1
                ...
                BEGIN
                    UPDATE  smi_t
                    SET     wStockMovementDtlRid = tmp.wStockMovementDtlRid ,
                    ...
                    FROM    [dbo].[eStockMovementItem] smi_t
                            INNER JOIN #sDataSet_SetStockMovementItem tmp ON smi_t.RowID = tmp.RowID
        ...
        IF OBJECT_ID('tempdb..#sDataSet_SetStockMovementItem') IS NOT NULL
            DROP TABLE #sDataSet_SetStockMovementItem;
    END;